...
def compute_pose_errors_and_print(df, rotation_deg, label):
    ...
    if PRINT_COLUMN_DATA:
        print(f"\nResults for {label} (Rotation: {rotation_deg}°):")
    ...
    for idx, row in df.iterrows():
        ...
        if PRINT_COLUMN_DATA:
            print(f"{idx:<5} {center[0]:10.3f} {center[1]:10.3f} {np.degrees(yaw_rad):10.2f} "
              f"{row['dx_odom']:8.3f} {row['dy_odom']:8.3f} {np.degrees(row['dyaw_odom']):10.2f}")
    ...
    if PRINT_STATS:
        ...
        print(f"  Mean Yaw Error (deg): {np.degrees(np.mean(yaw_errors)):.4f}, "
              f"Variance (deg²): {np.degrees(np.var(yaw_errors))**2:.4f}")
...
def plot_rotational_errors_by_speed():
    ...
    for file_path in file_list:
        ...
        label = f"{rotation_deg}°"
        x_err, y_err, yaw_err = compute_pose_errors_and_print(df, rotation_deg, label)

        grouped_data.setdefault(speed, []).append((rotation_deg, label, x_err, y_err, np.degrees(yaw_err)))

    for speed in sorted(grouped_data.keys()):
        group = sorted(grouped_data[speed], key=lambda x: x[0])

        x_err_all = [item[2] for item in group]
        y_err_all = [item[3] for item in group]
        yaw_err_all = [item[4] for item in group]
        ...
        plt.suptitle(f'Robot Pose Deviations - Speed: {speed:.2f} m/s')
        plt.tight_layout(rect=[0, 0.03, 1, 0.95])

        if SAVE_FIG:
            filename = f'rotational_errors_speed_{int(speed * 100)}.png'
